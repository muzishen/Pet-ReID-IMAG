...
# Backbone for res2net
_C.MODEL.BACKBONE.W = 26
_C.MODEL.BACKBONE.S = 4
...
# model type for convnext
_C.MODEL.BACKBONE.TYPE = ''
# use checkpoint to reduce GPU memory
...
_C.MODEL.LOSSES.CE.USE_PT = False
...
# Adaface Loss options
_C.MODEL.LOSSES.ADAFACE = CN()
_C.MODEL.LOSSES.ADAFACE.MARGIN = 0.25
_C.MODEL.LOSSES.ADAFACE.NUM_CLASSES = 128
_C.MODEL.LOSSES.ADAFACE.SCALE = 1.0
...
# BLUR
_C.INPUT.BLUR = CN({"ENABLED": False})
_C.INPUT.BLUR.KERNEL_SIZE = (5, 9)


# Random probability for image horizontal flip
_C.INPUT.VFLIP = CN({"ENABLED": False})
_C.INPUT.VFLIP.PROB = 0.5
...
# Custom
_C.TEST.SAVE_DIST = CN({"ENABLED": False})
