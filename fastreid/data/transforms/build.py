...
def build_transforms(cfg, is_train=True):
    ...
    if is_train:
        ...
        blur_kernel = cfg.INPUT.BLUR.KERNEL_SIZE

        # horizontal filp
        do_vflip = cfg.INPUT.VFLIP.ENABLED
        vflip_prob = cfg.INPUT.VFLIP.PROB
        ...
        if do_blur:
            res.append(T.GaussianBlur(kernel_size=blur_kernel, sigma=(0.1, 2.0)))
        ...
        if do_vflip:
            res.append(T.RandomVerticalFlip(p=vflip_prob))
        ...
        # if do_blur:
            # res.append(T.GaussianBlur(kernel_size=(5, 9), sigma=(0.1, 2.0)))
        # if do_crop:
